          	...
          	const previewImage = car.images[0]?.contentUrl ? "http://localhost:8080" + car.images[0].contentUrl : "https://via.placeholder.com/300";
					return (
						<Grid item xs={12} sm={6} md={4} key={car.id}>
							...
							>
								<Link to={`/cars/${car.id}`} style={{ textDecoration: "none", flex: 1 }}>
									<CardContent>
								...
								<CardMedia component="img" sx={{ width: "100%", height: 150, objectFit: "cover" }} image={previewImage} alt={car.title} />
								<Button variant="contained" color="error" onClick={() => handleDelete(car.id)} sx={{ m: 1 }}>
									Usuń ogłoszenie
								</Button>
							</Card>
						</Grid>
